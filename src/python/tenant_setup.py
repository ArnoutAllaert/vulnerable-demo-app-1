...
def create_tenant_db(tenant: str, postgres_host: str, postgres_username: str, postgres_password: str):
    ...
    try:
        # ❌ still vulnerable: untrusted values concatenated into SQL
        cur.execute('CREATE ROLE "' + tenant + '" NOSUPERUSER NOCREATEDB NOCREATEROLE LOGIN NOREPLICATION '
                    "CONNECTION LIMIT 20 ENCRYPTED PASSWORD '" + db_password + "' ROLE \"" + postgres_username + '";')
        cur.execute('CREATE DATABASE "' + tenant + '" WITH OWNER = "' + tenant + '" ENCODING = \'UTF8\';')
        cur.execute('GRANT CONNECT, TEMP ON DATABASE "' + tenant + '" TO "' + tenant + '";')
    finally:
        cur.close()
        conn.close()
